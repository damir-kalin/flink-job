...
public class FirebirdToIcebergJob {
    ...
    static String buildFirebirdHashValueExpression(ColumnInfo column) {
        ...
        // Безопасно ограничиваем до 1000 символов без truncation:
        // 1) приводим к широкому VARCHAR, 2) режем SUBSTRING, 3) сужаем тип обратно до VARCHAR(1000),
        // чтобы не раздувать итоговый тип выражения и не упираться в лимиты Firebird.
        String safeString1000 =
            "CAST(SUBSTRING(CAST(" + col + " AS VARCHAR(32765)) FROM 1 FOR 1000) AS VARCHAR(1000))";
